...
namespace KalkulatorPrzekroju
{
	...
	class RectangleSection : Section
	{
		...
		public override double ULS_ScinanieTotal(double NEd, double part, ULS_Set factors)
		{
			...
			z = d - 0.5*x;//Math.Max(0.5 * x, A2/Dimfactor);
			...
			double teta = factors.teta;
			double cotQ = Math.Tan(Math.PI / 2 - teta / 360 * 2 * Math.PI);
			
			NationalAnnex code = factors.Annex;

			if (code == NationalAnnex.BS)
			{
				if (NEd < 0 && cotQ > 1.25)         //p.6.2.3 BS-EN
				{
					cotQ = 1.25;
				}
			}
			...
			double alfa = this.MyStirrups.Alfa / 360 * (2 * Math.PI);
